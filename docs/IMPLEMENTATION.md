...
**작업 목록**:
- [ ] 이미지 포맷 감지 함수 구현
- [ ] WebP 변환 함수 구현 (`ConvertToWebP`)
  - [ ] 이미지 디코딩
  - [ ] WebP 인코딩 (품질 설정)
  - [ ] 메타데이터 보존 (선택적)
- [ ] 리사이징 함수 구현 (`ResizeImage`)
  - [ ] 프리셋 크기 지원
  - [ ] 커스텀 크기 지원
  - [ ] 비율 유지 옵션
- [ ] 변환 + 리사이징 통합 함수 (`ConvertAndResize`)
- [ ] 에러 처리 (지원하지 않는 포맷, 손상된 이미지 등)
- [ ] 메모리 최적화 (큰 이미지 처리)
...
**작업 목록**:
- [ ] 상태 구조체 정의 (`State` - LastProcessedTime 등)
- [ ] 상태 파일 경로 설정 (로컬 JSON 파일)
- [ ] 상태 로드 함수 구현 (`LoadState`)
- [ ] 상태 저장 함수 구현 (`SaveState`)
- [ ] 마지막 처리 시간 업데이트 함수
- [ ] 파일이 없을 때 초기화 처리
- [ ] 에러 처리 (파일 읽기/쓰기 실패)
...
**작업 목록**:
- [ ] 크론 스케줄러 초기화
- [ ] 크론 잡 함수 구현 (`ProcessImages`)
  - [ ] 상태에서 마지막 처리 시간 로드
  - [ ] R2에서 새 이미지 목록 조회 (마지막 처리 시간 이후)
  - [ ] 각 이미지에 대해:
    - [ ] 이미지 다운로드
    - [ ] WebP 변환
    - [ ] R2에 업로드 (원본 대체)
...
**작업 목록**:
- [ ] 변환 핸들러 구현 (`ConvertHandler`)
  - [ ] POST `/api/convert` 엔드포인트
  - [ ] GET `/api/convert` 엔드포인트 (쿼리 파라미터)
  - [ ] 요청 파싱 (JSON 또는 쿼리 파라미터)
  - [ ] 소스 타입 감지 (R2 키 vs URL)
  - [ ] 리사이징 파라미터 파싱 (width, height, preset)
  - [ ] 이미지 다운로드 (R2 또는 URL)
  - [ ] 이미지 변환 및 리사이징
  - [ ] R2에 업로드
  - [ ] 응답 반환 (성공/실패)
